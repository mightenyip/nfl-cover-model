#!/usr/bin/env python3
"""
Simple Run/Pass Matchup EPA Analysis
Analyzes matchup EPA broken down by run and pass using available data
"""
    ...
    try:
        ...
        for _, row in master_df.iterrows():
            ...
            if not fav_epa.empty and not dog_epa.empty:
                ...
                skipped_count += 1
                if fav_epa.empty:
                    print(f"Missing EPA data for {favorite} ({fav_abbr})")
                if dog_epa.empty:
                    print(f"Missing EPA data for {underdog} ({dog_abbr})")
        
        print(f"Processed {processed_count} games from Weeks 1-7")
        print(f"Skipped {skipped_count} games due to missing EPA data")
...
def create_visualizations(df, correlations):
    ...
    fig.suptitle('Run/Pass Matchup EPA vs ATS Performance Analysis', fontsize=16, fontweight='bold')
