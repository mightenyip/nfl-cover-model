#!/usr/bin/env python3
"""
Corrected Weighted Run/Pass Matchup EPA Analysis
Uses the user's specified weighting approach:
- Favorite-Weighted: Uses favorite's tendencies for both fav offense and dog defense
- Underdog-Weighted: Uses underdog's tendencies for both dog offense and fav defense
"""
    ...
    try:
        ...
        for _, row in master_df.iterrows():
            ...
            if not fav_epa.empty and not dog_epa.empty and not fav_ratio.empty and not dog_ratio.empty:
                ...
                # CORRECTED WEIGHTED Run-Pass Matchup EPA
                # Favorite-Weighted RP EPA: Uses favorite's tendencies for both fav offense and dog defense
                fav_pass_epa_diff = (fav_off_pass * fav_pass_pct) - (dog_def_pass * fav_pass_pct)
                fav_run_epa_diff = (fav_off_rush * fav_run_pct) - (dog_def_rush * fav_run_pct)
                fav_weighted_rp_epa = fav_pass_epa_diff + fav_run_epa_diff
                
                # Underdog-Weighted RP EPA: Uses underdog's tendencies for both dog offense and fav defense
                dog_pass_epa_diff = (fav_def_pass * dog_pass_pct) - (dog_off_pass * dog_pass_pct)
                dog_run_epa_diff = (fav_def_rush * dog_run_pct) - (dog_off_rush * dog_run_pct)
                dog_weighted_rp_epa = dog_pass_epa_diff + dog_run_epa_diff
                
                # Final Matchup EPA Diff: Difference between the two weighted approaches
                corrected_matchup_rp_epa_diff = fav_weighted_rp_epa - dog_weighted_rp_epa
                ...
                all_games.append({
                    ...
                    # CORRECTED Weighted Run-Pass EPA
                    'fav_weighted_rp_epa': fav_weighted_rp_epa,
                    'dog_weighted_rp_epa': dog_weighted_rp_epa,
                    'corrected_matchup_rp_epa_diff': corrected_matchup_rp_epa_diff,
                    
                    # Individual weighted components
                    'fav_pass_epa_diff': fav_pass_epa_diff,
                    'fav_run_epa_diff': fav_run_epa_diff,
                    'dog_pass_epa_diff': dog_pass_epa_diff,
                    'dog_run_epa_diff': dog_run_epa_diff,
...
def analyze_correlations(df):
    ...
    print("\n=== Corrected Weighted Run/Pass Matchup EPA vs ATS Correlation Analysis ===")
    print("=" * 90)
    ...
    # CORRECTED Weighted Run-Pass EPA
    correlations['fav_weighted_rp_epa'] = df['fav_weighted_rp_epa'].corr(df['margin_vs_spread'])
    correlations['dog_weighted_rp_epa'] = df['dog_weighted_rp_epa'].corr(df['margin_vs_spread'])
    correlations['corrected_matchup_rp_epa_diff'] = df['corrected_matchup_rp_epa_diff'].corr(df['margin_vs_spread'])
    
    # Individual weighted components
    correlations['fav_pass_epa_diff'] = df['fav_pass_epa_diff'].corr(df['margin_vs_spread'])
    correlations['fav_run_epa_diff'] = df['fav_run_epa_diff'].corr(df['margin_vs_spread'])
    correlations['dog_pass_epa_diff'] = df['dog_pass_epa_diff'].corr(df['margin_vs_spread'])
    correlations['dog_run_epa_diff'] = df['dog_run_epa_diff'].corr(df['margin_vs_spread'])
    
    # Cover rate correlations
    correlations['matchup_epa_vs_cover'] = df['matchup_epa_diff'].corr(df['favorite_covered'].astype(int))
    correlations['corrected_matchup_rp_epa_vs_cover'] = df['corrected_matchup_rp_epa_diff'].corr(df['favorite_covered'].astype(int))
    ...
    print()
    print("CORRECTED WEIGHTED RUN-PASS EPA:")
    print(f"  Favorite-Weighted RP EPA:                   {correlations['fav_weighted_rp_epa']:6.3f}")
    print(f"  Underdog-Weighted RP EPA:                   {correlations['dog_weighted_rp_epa']:6.3f}")
    print(f"  Corrected Matchup RP EPA Diff:              {correlations['corrected_matchup_rp_epa_diff']:6.3f}")
    print()
    print("INDIVIDUAL WEIGHTED COMPONENTS:")
    print(f"  Fav Pass EPA Diff (weighted):               {correlations['fav_pass_epa_diff']:6.3f}")
    print(f"  Fav Run EPA Diff (weighted):                {correlations['fav_run_epa_diff']:6.3f}")
    print(f"  Dog Pass EPA Diff (weighted):               {correlations['dog_pass_epa_diff']:6.3f}")
    print(f"  Dog Run EPA Diff (weighted):                {correlations['dog_run_epa_diff']:6.3f}")
    print()
    print("COVER RATE CORRELATIONS:")
    print(f"  Matchup EPA vs Cover:                       {correlations['matchup_epa_vs_cover']:6.3f}")
    print(f"  Corrected Matchup RP EPA vs Cover:          {correlations['corrected_matchup_rp_epa_vs_cover']:6.3f}")
    ...
    significance_tests = [
        ...
        ('corrected_matchup_rp_epa_diff', 'margin_vs_spread'),
        ('fav_weighted_rp_epa', 'margin_vs_spread'),
        ('dog_weighted_rp_epa', 'margin_vs_spread'),
        ('fav_pass_epa_diff', 'margin_vs_spread'),
        ('fav_run_epa_diff', 'margin_vs_spread'),
        ('dog_pass_epa_diff', 'margin_vs_spread'),
        ('dog_run_epa_diff', 'margin_vs_spread')
    ]
    ...
    for metric, target in significance_tests:
        ...
        print(f"{metric:40s}: p={p_value:.3f} {significance}")
    ...
    margin_correlations = {
        ...
        'Corrected Matchup RP EPA': correlations['corrected_matchup_rp_epa_diff'],
        'Fav-Weighted RP EPA': correlations['fav_weighted_rp_epa'],
        'Dog-Weighted RP EPA': correlations['dog_weighted_rp_epa']
    }
...
def create_visualizations(df, correlations):
    """Create visualizations for the corrected weighted run/pass EPA analysis"""
    print("\n=== Creating Corrected Weighted Run/Pass EPA Visualizations ===")
    print("=" * 65)
    ...
    fig.suptitle('Corrected Weighted Run/Pass Matchup EPA vs ATS Performance Analysis', fontsize=16, fontweight='bold')
    ...
    # 2. Corrected Weighted RP EPA vs Margin vs Spread
    ax2 = axes[0, 1]
    ax2.scatter(df['corrected_matchup_rp_epa_diff'], df['margin_vs_spread'], alpha=0.7, s=60, color='purple')
    ax2.axhline(y=0, color='red', linestyle='--', alpha=0.5)
    ax2.axvline(x=0, color='red', linestyle='--', alpha=0.5)
    ax2.set_xlabel('Corrected Matchup RP EPA Difference')
    ax2.set_ylabel('Margin vs Spread')
    ax2.set_title(f'Corrected Matchup RP EPA\n(r = {correlations["corrected_matchup_rp_epa_diff"]:.3f})')
    
    # Add trend line
    z = np.polyfit(df['corrected_matchup_rp_epa_diff'], df['margin_vs_spread'], 1)
    p = np.poly1d(z)
    ax2.plot(df['corrected_matchup_rp_epa_diff'], p(df['corrected_matchup_rp_epa_diff']), "r--", alpha=0.8)
    
    # 3. Favorite-Weighted vs Underdog-Weighted RP EPA
    ax3 = axes[0, 2]
    ax3.scatter(df['fav_weighted_rp_epa'], df['dog_weighted_rp_epa'], alpha=0.7, s=60, color='green')
    ax3.set_xlabel('Favorite-Weighted RP EPA')
    ax3.set_ylabel('Underdog-Weighted RP EPA')
    ax3.set_title('Fav-Weighted vs Dog-Weighted RP EPA')
    
    # Add trend line
    z = np.polyfit(df['fav_weighted_rp_epa'], df['dog_weighted_rp_epa'], 1)
    p = np.poly1d(z)
    ax3.plot(df['fav_weighted_rp_epa'], p(df['fav_weighted_rp_epa']), "r--", alpha=0.8)
    
    # 4. Correlation comparison bar chart
    ax4 = axes[1, 0]
    metrics = ['Overall', 'Pass', 'Rush', 'Simple RP', 'Corrected RP', 'Fav-Weighted', 'Dog-Weighted']
    corr_values = [
        ...
        correlations['corrected_matchup_rp_epa_diff'],
        correlations['fav_weighted_rp_epa'],
        correlations['dog_weighted_rp_epa']
    ]
    ...
    # 5. Individual weighted components
    ax5 = axes[1, 1]
    components = ['Fav Pass', 'Fav Run', 'Dog Pass', 'Dog Run']
    comp_values = [
        correlations['fav_pass_epa_diff'],
        correlations['fav_run_epa_diff'],
        correlations['dog_pass_epa_diff'],
        correlations['dog_run_epa_diff']
    ]
    colors = ['lightblue', 'lightgreen', 'lightcoral', 'lightyellow']
    
    bars = ax5.bar(components, comp_values, color=colors, alpha=0.7)
    ax5.set_ylabel('Correlation with Margin vs Spread')
    ax5.set_title('Individual Weighted Components')
    ...
    # 6. Corrected vs Simple RP EPA comparison
    ax6 = axes[1, 2]
    ax6.scatter(df['matchup_rp_epa_diff'], df['corrected_matchup_rp_epa_diff'], alpha=0.7, s=60, color='purple')
    ...
    ax6.set_ylabel('Corrected Matchup RP EPA')
    ax6.set_title('Corrected vs Simple RP EPA')
    ax6.legend()
    
    plt.tight_layout()
    
    # Save the plot
    output_path = 'images/corrected_weighted_matchup_rp_epa_analysis.png'
...
def generate_summary_report(df, correlations):
    ...
    print("\n=== CORRECTED WEIGHTED RUN/PASS MATCHUP EPA ANALYSIS REPORT ===")
    print("=" * 80)
    ...
    print(f"Corrected Matchup RP EPA:         {correlations['corrected_matchup_rp_epa_diff']:.3f}")
    print(f"Favorite-Weighted RP EPA:         {correlations['fav_weighted_rp_epa']:.3f}")
    print(f"Underdog-Weighted RP EPA:         {correlations['dog_weighted_rp_epa']:.3f}")
    print()
    ...
    margin_correlations = {
        ...
        'Simple Average RP EPA': correlations['matchup_rp_epa_diff'],
        'Corrected Matchup RP EPA': correlations['corrected_matchup_rp_epa_diff'],
        'Favorite-Weighted RP EPA': correlations['fav_weighted_rp_epa'],
        'Underdog-Weighted RP EPA': correlations['dog_weighted_rp_epa']
    }
    ...
    # Top games by corrected weighted RP EPA
    print("TOP 5 GAMES BY CORRECTED MATCHUP RP EPA ADVANTAGE:")
    print("-" * 60)
    top_games = df.nlargest(5, 'corrected_matchup_rp_epa_diff')
    for _, row in top_games.iterrows():
        print(f"{row['game']}: Corrected RP EPA = {row['corrected_matchup_rp_epa_diff']:.3f}, "
    ...
    if best_correlation > 0.3:
        ...
        print("3. Corrected weighted run/pass analysis provides valuable insights")
    elif best_correlation > 0.1:
        print("2. Moderate predictive power for spread performance")
        print("3. Some value in corrected weighted run/pass analysis")
    else:
        print("2. Limited predictive power for spread performance")
        print("3. Corrected weighted approach may not add significant value")
    
    print(f"4. Overall favorite cover rate: {favorite_cover_rate:.1%}")
    print(f"5. Analysis based on {total_games} games across 8 weeks")
    print("6. Corrected approach weights each team by their own run/pass tendencies")

def main():
    """Main analysis function"""
    print("=== Corrected Weighted Run/Pass Matchup EPA Analysis ===")
    print("Using user's specified weighting approach")
    print("=" * 80)
