#!/usr/bin/env python3
"""
Combined Week 1-5 ATS Analysis
Calculate overall performance from all five weeks
"""

import pandas as pd

def combine_week1_week5_ats():
    """Combine Week 1, 2, 3, 4, and 5 ATS results"""
    
    print("=== Combined Week 1-5 ATS Analysis ===")
    ...
    # Combine results
    combined_df = pd.concat([week1_df, week2_df, week3_df, week4_df, week5_df], ignore_index=True)
    ...
    print(f"\n=== Overall ATS Performance (Week 1-5) ===")
    ...
    # Calculate trend
    rates = [week1_rate, week2_rate, week3_rate, week4_rate, week5_rate]
    ...
    print(f"\n=== Recent Trend Analysis (Week 3-5) ===")
    recent_weeks = combined_df[combined_df['week'].isin([3, 4, 5])]
    ...
    if recent_rate > 50:
        print("📈 RECENT TREND: Underdogs performing better recently")
    elif recent_rate < 50:
        print("📉 RECENT TREND: Favorites performing better recently")
    else:
        print("⚖️ RECENT TREND: Balanced performance recently")
    
    # Save combined results
    output_file = "data/ats_trends/combined/combined_week1_week5_ats.csv"
...
def main():
    """Main function"""
    print("=== Combined Week 1-5 ATS Analysis ===")
    print("Calculating overall performance from all five weeks")
    print("=" * 60)
    
    df = combine_week1_week5_ats()
    
    print(f"\n=== Analysis Complete ===")
    print(f"📊 Combined ATS Performance: {df['underdog_covered'].sum()}/{len(df)} underdog covers")
    print(f"📁 Results saved to data/ats_trends/combined/combined_week1_week5_ats.csv")
