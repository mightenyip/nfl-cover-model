#!/usr/bin/env python3
"""
Generate Week 11 Predictions for Models A, B, and E
Using week11_EPA.csv data
"""
...
def run_model_a(week11_odds, epa_data):
    ...
    for _, game in week11_odds.iterrows():
...
def run_model_b(week11_odds, epa_data):
...
def run_model_e(week11_odds, epa_data):
...
def main():
    """Main function to generate Week 11 predictions"""
    
    print("="*80)
    print("WEEK 11 2025 PREDICTIONS - MODELS A, B, AND E")
    print("="*80)
    
    # Load Week 11 odds
    week11_odds = pd.read_csv("schedule/week11_2025_odds.csv")
    print(f"\nLoaded {len(week11_odds)} games for Week 11")
    
    # Load Week 11 EPA data
    epa_data = pd.read_csv("data/epa/week11/Week11_EPA.csv")
    print(f"Loaded EPA data for {len(epa_data)} teams")
    
    # Run all three models
    model_a_preds = run_model_a(week11_odds, epa_data)
    model_b_preds = run_model_b(week11_odds, epa_data)
    model_e_preds = run_model_e(week11_odds, epa_data)
    
    # Save individual model predictions
    model_a_preds.to_csv("models/model_a/model_a_week11_predictions.csv", index=False)
    model_b_preds.to_csv("models/model_b/model_b_week11_predictions.csv", index=False)
    model_e_preds.to_csv("models/model_e/model_e_week11_predictions.csv", index=False)
    
    print(f"\n✅ Model A predictions saved to models/model_a/model_a_week11_predictions.csv")
    print(f"✅ Model B predictions saved to models/model_b/model_b_week11_predictions.csv")
    print(f"✅ Model E predictions saved to models/model_e/model_e_week11_predictions.csv")
    
    # Create combined summary
    summary = week11_odds.copy()
    ...
    summary.to_csv("predictions/week11_all_models_predictions.csv", index=False)
    print(f"\n✅ Combined predictions saved to predictions/week11_all_models_predictions.csv")
