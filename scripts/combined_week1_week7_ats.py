#!/usr/bin/env python3
"""
Combined Week 1-7 ATS Analysis
Calculate overall performance from all seven weeks
"""

import pandas as pd

def combine_week1_week7_ats():
    """Combine Week 1, 2, 3, 4, 5, 6, and 7 ATS results"""
    
    print("=== Combined Week 1-7 ATS Analysis ===")
    ...
    week7_df = pd.read_csv("data/ats_results/week7/week7_ats_results.csv")
    week7_df['week'] = 7
    
    # Combine results
    combined_df = pd.concat([week1_df, week2_df, week3_df, week4_df, week5_df, week6_df, week7_df], ignore_index=True)
    ...
    week7_rate = (week7_df['underdog_covered'].sum() / len(week7_df)) * 100
    ...
    print(f"Week 7: {week7_rate:.1f}% underdog covers")
    
    # Calculate trend
    rates = [week1_rate, week2_rate, week3_rate, week4_rate, week5_rate, week6_rate, week7_rate]
    ...
    print(f"\n=== Recent Trend Analysis (Week 5-7) ===")
    recent_weeks = combined_df[combined_df['week'].isin([5, 6, 7])]
    ...
    # Week 7 specific analysis
    print(f"\n=== Week 7 Specific Analysis ===")
    week7_underdog_covers = week7_df['underdog_covered'].sum()
    week7_total = len(week7_df)
    week7_rate = week7_underdog_covers / week7_total * 100
    
    print(f"Week 7: {week7_underdog_covers}/{week7_total} underdog covers ({week7_rate:.1f}%)")
    
    if week7_rate < 30:
        print("🚨 EXTREME FAVORITE BIAS: Week 7 was heavily favorite-favored")
        print("  - This could indicate market correction")
        print("  - Model C should be cautious of over-favoring favorites")
    elif week7_rate > 70:
        print("🚨 EXTREME UNDERDOG BIAS: Week 7 was heavily underdog-favored")
        print("  - This could indicate market correction")
        print("  - Model C should be cautious of over-favoring underdogs")
    else:
        print("⚖️ BALANCED WEEK 7: Normal distribution")
    
    # Save combined results
    output_file = "data/ats_trends/combined/combined_week1_week7_ats.csv"
...
def main():
    """Main function"""
    print("=== Combined Week 1-7 ATS Analysis ===")
    print("Calculating overall performance from all seven weeks")
    print("=" * 60)
    
    df = combine_week1_week7_ats()
    
    print(f"\n=== Analysis Complete ===")
    print(f"📊 Combined ATS Performance: {df['underdog_covered'].sum()}/{len(df)} underdog covers")
    print(f"📁 Results saved to data/ats_trends/combined/combined_week1_week7_ats.csv")
