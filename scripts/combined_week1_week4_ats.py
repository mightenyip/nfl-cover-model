#!/usr/bin/env python3
"""
Combined Week 1-4 ATS Analysis
Calculate overall performance from all four weeks
"""

import pandas as pd

def combine_week1_week4_ats():
    """Combine Week 1, 2, 3, and 4 ATS results"""
    
    print("=== Combined Week 1-4 ATS Analysis ===")
    ...
    # Combine results
    combined_df = pd.concat([week1_df, week2_df, week3_df, week4_df], ignore_index=True)
    ...
    print(f"\n=== Overall ATS Performance (Week 1-4) ===")
    ...
    # Calculate trend
    rates = [week1_rate, week2_rate, week3_rate, week4_rate]
    ...
    output_file = "data/ats_trends/combined/combined_week1_week4_ats.csv"
...
def main():
    """Main function"""
    print("=== Combined Week 1-4 ATS Analysis ===")
    print("Calculating overall performance from all four weeks")
    print("=" * 60)
    
    df = combine_week1_week4_ats()
    
    print(f"\n=== Analysis Complete ===")
    print(f"📊 Combined ATS Performance: {df['underdog_covered'].sum()}/{len(df)} underdog covers")
    print(f"📁 Results saved to data/ats_trends/combined/combined_week1_week4_ats.csv")
