#!/usr/bin/env python3
"""
Regenerate Week 15 predictions with corrected EPA calculation
Save to separate files for comparison
"""
...
def load_epa_data():
    """Load Week 15 EPA data"""
    try:
        epa_df = pd.read_csv("data/epa/week15/Week15_EPA.csv")
        print(f"✅ Loaded EPA data for {len(epa_df)} teams")
        return epa_df
    except FileNotFoundError:
        print("❌ Error: data/epa/week15/Week15_EPA.csv not found")
        return None

def load_week15_odds():
    """Load Week 15 odds"""
    try:
        odds_df = pd.read_csv("schedule/week15_2025_odds.csv")
        print(f"✅ Loaded {len(odds_df)} games from Week 15 odds")
        return odds_df
    except FileNotFoundError:
        print("❌ Error: schedule/week15_2025_odds.csv not found")
        return None

def run_model_a_corrected(week15_odds, epa_data):
    """Run Model A predictions with corrected EPA calculation"""
    team_mapping = get_team_mapping()
    predictions = []
    
    for idx, row in week15_odds.iterrows():
        ...
        # CORRECTED: Calculate from underdog's perspective
...
def run_model_b_corrected(week15_odds, epa_data):
    """Run Model B predictions with corrected EPA calculation"""
    team_mapping = get_team_mapping()
    predictions = []
...
def create_predictions_final_corrected(week15_odds, model_a, model_b, model_e):
    """Create predictions_final CSV with corrected predictions"""
...
def main():
    """Main function"""
    print("="*80)
    print("REGENERATING WEEK 15 PREDICTIONS (CORRECTED EPA CALCULATION)")
    print("="*80)
    
    # Load data
    week15_odds = load_week15_odds()
    if week15_odds is None:
        return
    ...
    print("\n=== Generating Model A Predictions (CORRECTED) ===")
    model_a = run_model_a_corrected(week15_odds, epa_data)
    print(f"✅ Model A: {len(model_a)} predictions")
    
    print("\n=== Generating Model B Predictions (CORRECTED) ===")
    model_b = run_model_b_corrected(week15_odds, epa_data)
    print(f"✅ Model B: {len(model_b)} predictions")
    
    print("\n=== Generating Model E Predictions ===")
    model_e = run_model_e(week15_odds)
    print(f"✅ Model E: {len(model_e)} predictions")
    
    # Save individual model predictions to corrected folder
    os.makedirs("models/model_a/corrected", exist_ok=True)
    os.makedirs("models/model_b/corrected", exist_ok=True)
    os.makedirs("models/model_e/corrected", exist_ok=True)
    
    model_a.to_csv("models/model_a/corrected/model_a_week15_predictions_corrected.csv", index=False)
    model_b.to_csv("models/model_b/corrected/model_b_week15_predictions_corrected.csv", index=False)
    model_e.to_csv("models/model_e/corrected/model_e_week15_predictions_corrected.csv", index=False)
    
    print("\n✅ Saved individual model predictions to corrected folders")
    
    # Create predictions_final
    print("\n=== Creating Predictions Final (CORRECTED) ===")
    predictions_final = create_predictions_final_corrected(week15_odds, model_a, model_b, model_e)
    
    # Save to predictions folder with corrected suffix
    os.makedirs("predictions", exist_ok=True)
    output_file = "predictions/week15_predictions_final_corrected.csv"
    ...
    print(f"\n✅ Week 15 corrected predictions saved to {output_file}")
