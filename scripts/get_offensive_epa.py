#!/usr/bin/env python3
"""
Get Offensive EPA Data - Extract latest offensive EPA data from SumerSports website
"""

import pandas as pd
from datetime import datetime

def create_offensive_epa_data():
    """Create offensive EPA data based on SumerSports website data"""
    
    # Data from SumerSports offensive page (as of 10-15-2025)
    ...
    df = pd.DataFrame(offensive_data, columns=[
        ...
        'epa_pass_off', 'epa_rush_off', 'pass_yards', 'comp_pct',
        'pass_td', 'rush_yards', 'rush_td', 'adot',
    ...
    df['source'] = 'sumersports_offensive_manual'
...
def main():
    """Main function to create offensive EPA data"""
    
    print("=== Offensive EPA Data Creation ===")
    
    # Create offensive data
    ...
    print(f"Teams: {len(offensive_df)}")
    print(f"Last Updated: {offensive_df['last_updated'].iloc[0]}")
    
    # Top 5 offensive teams
    print(f"\nTop 5 Offensive Teams:")
    top_off = offensive_df.nlargest(5, 'epa_off_per_play')[['team_name', 'epa_off_per_play', 'epa_pass_off', 'epa_rush_off']]
    print(top_off.to_string(index=False))
    
    # Worst 5 offensive teams
    print(f"\nWorst 5 Offensive Teams:")
    worst_off = offensive_df.nsmallest(5, 'epa_off_per_play')[['team_name', 'epa_off_per_play', 'epa_pass_off', 'epa_rush_off']]
    print(worst_off.to_string(index=False))
    
    print(f"\n✅ Successfully created updated offensive EPA data!")
    print(f"📊 Data includes {len(offensive_df)} teams with latest EPA/Pass and EPA/Rush offensive statistics")
