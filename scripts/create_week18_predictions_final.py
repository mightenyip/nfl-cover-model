#!/usr/bin/env python3
"""
Create Week 18 Predictions Final (2025)
...
Outputs:
- models/model_a/model_a_week18_predictions.csv
- models/model_b/model_b_week18_predictions.csv
- models/model_e/model_e_week18_predictions.csv
- predictions/week18_predictions_final.csv
"""
...
def load_week18_odds():
    try:
        odds_df = pd.read_csv("schedule/week18_2025_odds.csv")
        print(f"✅ Loaded {len(odds_df)} games from Week 18 odds")
        return odds_df
    except FileNotFoundError:
        print("❌ Error: schedule/week18_2025_odds.csv not found")
        return None
...
def create_predictions_final(
    week_odds: pd.DataFrame, model_a: pd.DataFrame, model_b: pd.DataFrame, model_e: pd.DataFrame
) -> pd.DataFrame:
...
def main():
    print("=" * 80)
    print("WEEK 18 2025 PREDICTIONS FINAL")
    print("=" * 80)

    week_odds = load_week18_odds()
    ...
    model_a.to_csv("models/model_a/model_a_week18_predictions.csv", index=False)
    model_b.to_csv("models/model_b/model_b_week18_predictions.csv", index=False)
    model_e.to_csv("models/model_e/model_e_week18_predictions.csv", index=False)
    ...
    output_file = "predictions/week18_predictions_final.csv"
