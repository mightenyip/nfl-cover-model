#!/usr/bin/env python3
"""
Model C Week 6 Updated: Spread Rules with Real-World ATS Trends
Based on Week 1-5 performance data
"""
...
def run_model_c_week6_updated():
    """Run updated Model C for Week 6 using spread-based rules with real ATS trends"""
    
    print("=== Week 6 Model C Updated: Spread Rules + ATS Trends ===")
    print("Incorporating updated ATS performance data through Week 5:")
    ...
    # Load Week 6 schedule and odds
    week6_odds_path = "../../schedule/week6_2025_odds.csv"
    
    # Check if file exists
    if not os.path.exists(week6_odds_path):
        print(f"Error: {week6_odds_path} not found")
        return None
    ...
    for index, row in week6_odds.iterrows():
    ...
    # Create DataFrame
    week6_data = pd.DataFrame(predictions)

    print(f"\n=== Week 6 Model C Updated Predictions ===")
    
    # Group by rule type
    rule_groups = week6_data.groupby('rule_applied').size()
    
    for rule, count in rule_groups.items():
        rule_games = week6_data[week6_data['rule_applied'] == rule]
        print(f"\n{rule} ({count} games):")
    ...
    print(f"Underdog Covers: {sum(week6_data['predicted_cover'])} ({sum(week6_data['predicted_cover'])/len(week6_data):.1%})")
    print(f"Favorite Covers: {sum(~week6_data['predicted_cover'])} ({sum(~week6_data['predicted_cover'])/len(week6_data):.1%})")
    
    high_conf = week6_data[week6_data['confidence'] == 'HIGH']
    ...
    # Save predictions
    week6_data.to_csv("model_c_week6_updated_predictions.csv", index=False)
    print(f"\n✅ Updated Model C Week 6 predictions saved to: model_c_week6_updated_predictions.csv")

    return week6_data

if __name__ == "__main__":
    run_model_c_week6_updated()
